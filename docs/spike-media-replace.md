...
**切入主线必带项：** Mirror 装站见 [`tools/mirror-install/`](../tools/mirror-install/)（`make-package.ps1` + Win7 双击 `Install-RoadDeskMirror.bat`）。Host **管理员提权产品化**仍待办（任务 3；Mirror scrub HKLM；非提升 + 开 DM = 键鼠死）。见 RESULTS「迁移待办：Host 管理员权限」与 [coexistence.md](../spikes/mirror-driver/docs/coexistence.md)。
