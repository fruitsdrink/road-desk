...
static VOID RdmMaybeDirtyDst(SURFOBJ* psoDst, RECTL* prcl) {
  MIRRSURF* m;
  if (!psoDst || !psoDst->dhsurf || !prcl) {
    return;
  }
  m = (MIRRSURF*)psoDst->dhsurf;
  if (m->bIsScreen) {
    RdmDirtyAddRect(prcl);
  }
}
...
DHPDEV dhpdev)
{
    ...
    {
      ULONG pitch = ppdev->lDeltaScreen;
      if (pitch == 0) {
        pitch = ppdev->cxScreen * ((ppdev->ulBitCount + 7) / 8);
      }
      RdmDirtyInit(ppdev->cxScreen, ppdev->cyScreen, pitch, ppdev->ulBitCount);
    }
   ...
   )
{
   ...
   RdmMaybeDirtyDst(psoDst, prclDst);
   ...
   )
{
   ...
   if (prclOpaque) {
     RdmMaybeDirtyDst(psoDst, prclOpaque);
   } else if (prclExtra) {
     RdmMaybeDirtyDst(psoDst, prclExtra);
   }
...
MIX        mix)
{
    ...
    RdmMaybeDirtyDst(pso, prclBounds);
          ...
          PVOID pvOut)
{
    ...
    if (iEsc == RDM_ESC_GET_DIRTY) {
      ulRet = RdmDirtyFetch(pvOut, cjOut);
      return ulRet;
    }
